...
/****** Interface *******/
/************************/
function createDoubleGauge(data, reload)
{
	...
	var gauge = new DoubleGauge(config.name + "Graphic", config, data);
	gauge.render();
	...
	var gauge2 = new DoubleGauge(config.name + "Graphic", config, data);
	gauge2.render();
	_graphics.push(gauge2);

	for(var i = 0; i < _graphics.length; i++)
	{
		if(_graphics[i].config.type == "circle-a" && _graphics[i].placeholderName == config.name + "Graphic")
		{
			var value = $("#" + config.name + "Graphic select.date-gauge").val().split("x")[0];
			_graphics[i].redraw(value);
		}
		else if(_graphics[i].config.type == "circle-b" && _graphics[i].placeholderName == config.name + "Graphic")
		{
			var value = $("#" + config.name + "Graphic select.date-gauge").val().split("x")[1];
			_graphics[i].redraw(value);
		}
	}
}
...
/****** Class *******/
/********************/
function DoubleGauge(placeholderName, configuration, data)
{
	...
	this.configure = function(configuration)
	{
		...
		this.config.type = configuration.type;
		this.config.optHtml = configuration.optHtml || false;
		this.config.width = configuration.width || _graphicSize.width / 2;
	...
	this.render = function()
	{
		...
		this.body = d3.select("#" + this.placeholderName)
							.append("svg:svg")
								.attr("class", "gauge " + this.config.type)
		...
		var pointerContainer = this.body.append("svg:g").attr("class", "pointerContainer " + this.config.type);
		...
		pointerContainer.selectAll("text")
							...
							.enter()
								.append("svg:text")
									.attr("x", this.config.cx)
									.attr("y", this.config.size - this.config.cy / 4 - fontSize - 20)
									.attr("dy", fontSize / 2)
									.attr("text-anchor", "middle")									
									.style("font-size", fontSize + "px")
									.style("fill", "#EE4037")
		...
		if(data.answers.length > 0)
		{
			if(this.config.optHtml)
			{
				...
				{					
					for(var i = 0; i < _graphics.length; i++)
					{			
						if(_graphics[i].config.type == "circle-a" && _graphics[i].placeholderName == placeholderName)
						{
							var value = $("#" + placeholderName + " select.date-gauge").val().split("x")[0];
							_graphics[i].redraw(value);
						}
						else if(_graphics[i].config.type == "circle-b" && _graphics[i].placeholderName == placeholderName)
						{
							var value = $("#" + placeholderName + " select.date-gauge").val().split("x")[1];
							_graphics[i].redraw(value);
						}		
					}
				});
